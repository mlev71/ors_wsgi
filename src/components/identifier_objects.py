...
class Ark(CoreMetadata):
    ...
    def importAPI(self):
        ...
        ezid_response = requests.get(url = "https://ezid.cdlib.org/id/"+self.GUID)
        self.data = convertArkToJson(api_response.content.decode('utf-8'))
        
        neo_post = self.postNeo()

        response_message = {"cache": {"imported": neo_post} }
...
class Doi(CoreMetadata): 
    ...
    def getNeo(self):
        ''' Query the Neo Cache, retrieve all 
