...
def parseCloudLocation(content, relatedIds):
    ''' Add cloud location to XML
    ''' 
    aws_location = list(filter(lambda x: re.match('aws', x), content))
    gpc_location = list(filter(lambda x: re.match('gpc', x), content))

    for aws in aws_location:
        etree.SubElement(relatedIds, "relatedIdentifier", relatedIdentifierType="URL", 
                         relationType= "HasMetadata")._setText(aws)


    for gpc in gpc_location:
        etree.SubElement(relatedIds, "relatedIdentifier", relatedIdentifierType="URL", 
                         relationType= "HasMetadata")._setText(aws)
