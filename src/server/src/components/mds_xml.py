...
def outputDataciteXML(doi_json):
    ...
    resource = E.resource(
        ...
        E.resourceType(doi_json.get('@type'), resourceTypeGeneral=doi_json.get('@type'))
    )
    ...
    # Url homepage
    # and includedInDatacatalog
    url = doi_json.get('url')
    dc = doi_json.get('includedInDataCatalog')

    if url is not None or dc is not None:
...
def unpack_xml(input_root):
    ...
    # datepublished
    datePublished = input_root.findtext('{http://datacite.org/schema/kernel-4}version')
